...
class SVG(object):
    ...
    def parse_doc(self):
        ...
        self.width = self.parse_float(wm)
        self.height = self.parse_float(hm)
        ...
        if e.tag.endswith('path'):
            ...
            while pathdata:
                ...
                if opcode == 'M':
                    ...
                    mx = 2 * self.x - self.last_cx
                    my = 2 * self.y - self.last_cy
                    x,y = pnext()
                    ...
                    my = 2 * self.y - self.last_cy
                    x,y = pnext()
                    self.quadratic_curve_to(mx+self.x, my+self.y, x+self.x, y+self.y)
            ...
            self.warn("Text tag not supported")
